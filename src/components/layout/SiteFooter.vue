<template>
  <footer class="bg-deep-forest text-white mt-auto">
    <!-- Main Footer Content -->
    <div class="container-wide py-12 lg:py-16">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
        <!-- Brand Section -->
        <div class="space-y-4">
          <div class="flex items-center mb-4">
            <div class="w-12 h-12 bg-warm-gold rounded-full flex items-center justify-center mr-3">
              <span class="text-rich-brown font-bold text-xl">BE</span>
            </div>
            <div>
              <h3 class="font-serif text-xl font-bold">Blackberry</h3>
              <p class="text-sm text-sage-green">Equestrian</p>
            </div>
          </div>
          <p class="text-cream/80 leading-relaxed">
            Premier Hunter/Jumper boarding and training facility serving the Durham Region with excellence since 2010.
          </p>
          
          <!-- Social Media Links -->
          <div v-if="hasSocialMedia" class="flex space-x-3">
            <a
              v-if="siteSettings.social.facebook"
              :href="siteSettings.social.facebook"
              target="_blank"
              rel="noopener noreferrer"
              class="social-link"
              aria-label="Follow us on Facebook"
            >
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
            </a>
            <a
              v-if="siteSettings.social.instagram"
              :href="siteSettings.social.instagram"
              target="_blank"
              rel="noopener noreferrer"
              class="social-link"
              aria-label="Follow us on Instagram"
            >
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zM5.838 12a6.162 6.162 0 1112.324 0 6.162 6.162 0 01-12.324 0zM12 16a4 4 0 110-8 4 4 0 010 8zm4.965-10.405a1.44 1.44 0 112.881.001 1.44 1.44 0 01-2.881-.001z"/>
              </svg>
            </a>
            <a
              v-if="siteSettings.social.youtube"
              :href="siteSettings.social.youtube"
              target="_blank"
              rel="noopener noreferrer"
              class="social-link"
              aria-label="Subscribe to our YouTube channel"
            >
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
              </svg>
            </a>
          </div>
        </div>

        <!-- Quick Links -->
        <div>
          <h4 class="footer-heading">Quick Links</h4>
          <ul class="space-y-2">
            <li>
              <RouterLink to="/services" class="footer-link">
                Our Services
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/horses" class="footer-link">
                Horses for Sale
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/facilities" class="footer-link">
                Facilities
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/team" class="footer-link">
                Meet the Team
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/blog" class="footer-link">
                Blog & News
              </RouterLink>
            </li>
          </ul>
        </div>

        <!-- Contact Info -->
        <div v-if="hasContactInfo">
          <h4 class="footer-heading">Contact Us</h4>
          <div class="space-y-3">
            <div v-if="siteSettings.contact.address" class="flex items-start">
              <MapPinIcon class="w-5 h-5 text-warm-gold mt-0.5 mr-2 flex-shrink-0" />
              <address class="text-cream/80 not-italic">
                {{ siteSettings.contact.address }}
              </address>
            </div>
            <div v-if="siteSettings.contact.phone" class="flex items-center">
              <PhoneIcon class="w-5 h-5 text-warm-gold mr-2 flex-shrink-0" />
              <a
                :href="`tel:${siteSettings.contact.phone}`"
                class="footer-link"
              >
                {{ siteSettings.contact.phone }}
              </a>
            </div>
            <div v-if="siteSettings.contact.email" class="flex items-center">
              <EnvelopeIcon class="w-5 h-5 text-warm-gold mr-2 flex-shrink-0" />
              <a
                :href="`mailto:${siteSettings.contact.email}`"
                class="footer-link"
              >
                {{ siteSettings.contact.email }}
              </a>
            </div>
          </div>
        </div>

        <!-- Business Hours & CTA -->
        <div>
          <h4 class="footer-heading">Visit Us</h4>
          <div class="space-y-4">
            <div class="text-cream/80">
              <p class="font-medium text-cream mb-2">Business Hours:</p>
              <p v-if="siteSettings.contact.hours">
                {{ siteSettings.contact.hours }}
              </p>
              <p v-else>
                Mon-Fri: 8:00 AM - 8:00 PM<br>
                Sat-Sun: 8:00 AM - 6:00 PM
              </p>
            </div>
            
            <!-- Call to Action -->
            <BaseButton
              variant="primary"
              size="md"
              full-width
              @click="$router.push('/contact')"
            >
              Schedule a Visit
            </BaseButton>
          </div>
        </div>
      </div>
    </div>

    <!-- Bottom Bar -->
    <div class="bg-charcoal/30 border-t border-white/10">
      <div class="container-wide py-4">
        <div class="flex flex-col md:flex-row justify-between items-center space-y-2 md:space-y-0">
          <p class="text-sm text-cream/60">
            © {{ new Date().getFullYear() }} Blackberry Equestrian. All rights reserved.
          </p>
          <div class="flex space-x-6 text-sm">
            <RouterLink to="/privacy" class="footer-link">
              Privacy Policy
            </RouterLink>
            <RouterLink to="/terms" class="footer-link">
              Terms of Service
            </RouterLink>
            <a href="/sitemap.xml" class="footer-link">
              Sitemap
            </a>
          </div>
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup>
import { RouterLink } from 'vue-router'
import { useMainStore } from '@/stores/main'
import { storeToRefs } from 'pinia'
import BaseButton from '@/components/ui/BaseButton.vue'
import {
  MapPinIcon,
  PhoneIcon,
  EnvelopeIcon
} from '@heroicons/vue/24/outline'

const mainStore = useMainStore()
const { siteSettings, hasContactInfo, hasSocialMedia } = storeToRefs(mainStore)
</script>

<style scoped>
.footer-heading {
  @apply text-warm-gold font-semibold text-lg mb-4;
}

.footer-link {
  @apply text-cream/80 hover:text-warm-gold transition-colors duration-200;
}

.social-link {
  @apply inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/10 text-cream hover:bg-warm-gold hover:text-rich-brown transition-all duration-200 transform hover:scale-110;
}
</style>